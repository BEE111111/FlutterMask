...
class BibEntry {
  ...
  factory BibEntry.fromJson(Map<String, dynamic> json) {
    return BibEntry(
      ...
      number: json['number'] ?? '',
      issn: json['issn'] ?? '',
      index: (json['index'] != null && json['index'] is! String) ? json['index'].toDouble() : 0.0,
      type: json['type'] ?? '',
      note: json['note'] ?? '',
      isbn: json['isbn'] ?? '',
      language: json['language'] ?? '',
      ball: (json['ball'] != null && json['ball'] is! String) ? json['ball'].toDouble() : 0.0,
    );
  }
...
Future<Scientist?> fetchScientist(String username) async {
  ...
  if (response.statusCode == 200) {
    ...
    // Return null if fetching the scientist fails
    return null;
  }
}

Future<BibEntry> fetchBibEntry(String entryId) async {
  final response = await http.get(Uri.parse('http://127.0.0.1:8000/bibtex/doi/${entryId}/user/$username'));
  if (response.statusCode == 200) {
    final decodeResponse = utf8.decode(response.bodyBytes);
    Map<String, dynamic> jsonMap = json.decode(decodeResponse);
    return BibEntry.fromJson(jsonMap); // Parse the JSON object directly
  } else {
    throw Exception('Failed to load entry. Status code: ${response.statusCode}');
  }
}

Future<void> saveBibEntry(BuildContext context, BibEntry entry, String id_) async {
  final response = await http.patch(
    Uri.parse('http://127.0.0.1:8000/bibtex/check/${id_}/'),
...
String getOtherAuthor(String author, String firstAuthor) {
  var authorsList = author.split(' and ');
  // Remove the first author from the list
  authorsList.remove(firstAuthor);
  // Join the remaining authors with ' and '
  return authorsList.join(' and ');
}


double calculateBalls(double index, String language,String type, String note, String number_of_authors, String pages, String isbn ) {
  double balls = 0;
  if (type.isNotEmpty) {
    if (type == 'journal') {
      if (index != 0 ) {
        double indexValue = index;
        if ((indexValue! >= 0.27 && language == 'Foreign') || (indexValue >= 0.133 && language == 'Russian')) {
          if (language == 'Russian') {
            balls = (indexValue * 60);
          } else if (language == 'Foreign') {
            balls = (indexValue * 30);
          }
        } else if ((indexValue < 0.27 && language == 'Foreign') || (indexValue < 0.133 && language == 'Russian')) {
          balls = 8;
        }
      } else {
        balls = 8;
...
class BibEntryEditPage extends StatefulWidget {
  final String entryId;
  final String id_;
  final String username;
  const BibEntryEditPage({Key? key, required this.entryId, required this.id_,  required this.username}) : super(key: key);

  @override
  _BibEntryEditPageState createState() => _BibEntryEditPageState();
}

class _BibEntryEditPageState extends State<BibEntryEditPage> {
  ...
  late TextEditingController _ballContoller;
  ...
  void initState() {
    ...
    _ballContoller = TextEditingController();
    _loadData();
  }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _authorinrussianController.text = scientist?.author_in_russian ?? _currentEntry.author_in_russian;
          _first_authorController.text = scientist?.first_author ?? getFirstAuthor(_authorController.text);
          _organisationController.text =  _currentEntry.organization;
          _subdivisionController.text = _currentEntry.subdivision;
          _relationsController.text = scientist?.relations ?? _currentEntry.relations;
          print("Scientist relations: ${scientist?.relations}");
          print("Current entry relations: ${_currentEntry.relations}");
          _other_authorsController.text = getOtherAuthor(_authorController.text,_first_authorController.text );
          _number_of_authorsController.text = countAuthors(_authorController.text).toString();
          _booktitleController.text = _currentEntry.booktitle;
          _quartileController.text = _currentEntry.quartile;
          _number_of_affiliationsController.text = _currentEntry.number_of_affiliation.toString();
          ...
          _ballContoller.text = _currentEntry.ball.toString();
          _isLoading = false;
        });
      }
    } catch (e) {
                  ...
                  children: [
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        labelStyle: TextStyle(color: Colors.black, fontSize: 16, fontWeight: FontWeight.w400),
                      ),
                      validator: (value) {
                        return null;
                      },
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Эксперимент, выполненный в России?',
                        focusedBorder: UnderlineInputBorder(
                            borderSide: BorderSide(color: Colors.brown, width: 1.5)),
                        labelStyle: TextStyle(color: Colors.black, fontSize: 16, fontWeight: FontWeight.w400),
                      ),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: _quartiles.map<DropdownMenuItem<String>>((String value) {
                        ...
                        child: MaterialButton(
                          ...
                          onPressed: () async {
                            if (_formKey.currentState!.validate()) {
                              ...
                              _currentEntry.publisher = _publisherController.text;
                              ...
                              _currentEntry.volume = int.tryParse(_volumeController.text) ?? 0;
                              _currentEntry.year = int.tryParse(_yearController.text) ?? 0;
                              _currentEntry.author_in_russian = _authorinrussianController.text;
                              _currentEntry.first_author = _first_authorController.text;
                              _currentEntry.organization = _organisationController.text;
                              _currentEntry.subdivision = _subdivisionController.text;
                              _currentEntry.relations = _relationsController.text;
                              _currentEntry.other_authors = _other_authorsController.text;
                              _currentEntry.number_of_authors = _number_of_authorsController.text;
                              _currentEntry.booktitle = _booktitleController.text;
                              _currentEntry.quartile = _selectedQuartile ?? '';
                              _currentEntry.issn = _issnController.text;
                              _currentEntry.number_of_affiliation = int.tryParse(_number_of_affiliationsController.text) ?? 0;
                              _currentEntry.number_of_theme = _number_of_themeController.text;
                              _currentEntry.gratitude = _gratitudeController.text;
                              _currentEntry.number = _numberController.text;
                              _currentEntry.index = double.tryParse(_indexController.text) ?? 0.0;
                              ...
                              double p = calculateBalls(_currentEntry.index, _currentEntry.language ,_currentEntry.type, _currentEntry.note,_currentEntry.number_of_authors,_currentEntry.pages, _currentEntry.isbn);
                              _currentEntry.ball =  p;
                              // Update other fields of _currentEntry
                              try {
                                await saveBibEntry(context, _currentEntry, widget.id_);
                              } catch (e) {
                                print(e);
                              }
                            }
  ...
  void dispose() {
    ...
    _ballContoller.dispose();
    super.dispose();
  }
}

void main() {
  runApp(MaterialApp(home: BibEntryEditPage(entryId: 'some_entry_id', id_: 'some_id', username: username,))); // Use actual entryId and id_
}
